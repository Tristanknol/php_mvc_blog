    ...
    <link rel="stylesheet" href="dist/output.css">
     ...
     <header class="px-4 lg:px-20 bg-purple-900 flex items-center justify-between">
         ...
         if (isset($_SESSION["loggedIn"]) && $_SESSION["loggedIn"] === TRUE) {
             echo "<a href='?controller=user&action=showProfile' class='text-white text-md sm:text-xl lg:text-2xl xl:text-3xl'>Profile</a>";
         } else {
             echo "<a href='?controller=user&action=showLogin' class='text-white text-md sm:text-xl lg:text-2xl xl:text-3xl'>Login</a>";
         }
         ?>
     </header>
